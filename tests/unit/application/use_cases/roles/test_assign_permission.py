...
class FakeRoleRepository:
    def __init__(self, role: Role | None) -> None:
        self.role = role
        self.assigned_permission_id: UUID | None = None
    ...
    def assign_permission(
        ...
        self.assigned_permission_id = permission.id
...
def test_assign_permission_adds_permission_and_commits() -> None:
    actor = make_actor()
    ...
    assert any(existing.id == permission.id for existing in result.permissions)

    assert uow.roles.assigned_permission_id == permission.id
    assert uow.committed is True


def test_assign_permission_rejects_missing_role() -> None:
    actor = make_actor()
    ...
    assert uow.roles.assigned_permission_id is None
    assert uow.committed is False


def test_assign_permission_rejects_actor_who_cannot_manage_role() -> None:
...
def test_assign_permission_rejects_missing_permission() -> None:
    actor = make_actor()
    ...
    assert target_role.permissions == []
    assert uow.roles.assigned_permission_id is None
    assert uow.committed is False


def test_assign_permission_rejects_permission_already_assigned() -> None:
    actor = make_actor()
